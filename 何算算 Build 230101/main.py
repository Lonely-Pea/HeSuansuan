...
# 常量
TITLE = "何算算 Build 230101 By Lonely-Pea"  #  窗口标题
...
帮助信息
一、关于软件
1. 软件名称: 何算算 build 230101
2. 软件作者: Lonely-Pea
3. 软件使用操作系统: windows XP, Windows Vista, Windows 7, Windows 8, Windows 8.1, Windows 10, Windows 11及更高的Windows操作系统
4. 软件收费: 免费
5. 软件官网: 暂无
...
感谢QQ号为1349933474的人对该软件提供的部分技术支持！
"""
...
class CalcFrame(tk.Frame):
    ...
    # 计算器界面
    def calc(self):
        frame = ttk.LabelFrame(self, text="计算器")
        frame.place(x=0, y=0, width=NOTEBOOK_WIDTH, height=NOTEBOOK_HEIGHT)

        tk.Label(frame, text="输入框:", anchor="w").place(x=0, y=0, width=LABEL_WIDTH, height=LABEL_HEIGHT)
        tk.Entry(frame, textvariable=self.entry_var).place(x=LABEL_WIDTH, y=0, width=ENTRY_WIDTH, height=ENTRY_HEIGHT)  
        
        # 按钮
        Button(frame, text="(", command=lambda x: self.entry_var.set(self.entry_var.get() + "(")).place(x=0, y=LABEL_HEIGHT+1, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text=")", command=lambda x: self.entry_var.set(self.entry_var.get() + ")")).place(x=BUTTON_WIDTH + 1, y=LABEL_HEIGHT+1, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="C", command=lambda x: self.entry_var.set("")).place(x=BUTTON_WIDTH*2 + 2, y=LABEL_HEIGHT+1, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="<-", command=lambda x: self.entry_var.set(self.entry_var.get()[0: len(self.entry_var.get())-1])).place(x=BUTTON_WIDTH*3 + 3, y=LABEL_HEIGHT+1, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="√", command=lambda x: self.entry_var.set(self.entry_var.get() + "**0.5")).place(x=BUTTON_WIDTH*4 + 4, y=LABEL_HEIGHT+1, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)

        Button(frame, text="7", command=lambda x: self.entry_var.set(self.entry_var.get() + "7")).place(x=0, y=LABEL_HEIGHT * 2+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="8", command=lambda x: self.entry_var.set(self.entry_var.get() + "8")).place(x=BUTTON_WIDTH + 1, y=LABEL_HEIGHT * 2+2, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="9", command=lambda x: self.entry_var.set(self.entry_var.get() + "9")).place(x=BUTTON_WIDTH*2+2, y=LABEL_HEIGHT*2+2, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="÷", command=lambda x: self.entry_var.set(self.entry_var.get() + "/")).place(x=BUTTON_WIDTH*3+3, y=LABEL_HEIGHT*2+2, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="x²", command=lambda x: self.entry_var.set(self.entry_var.get() + "**2")).place(x=BUTTON_WIDTH*4+4, y=LABEL_HEIGHT*2+2, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)

        Button(frame, text="4", command=lambda x: self.entry_var.set(self.entry_var.get() + "4")).place(x=0, y=LABEL_HEIGHT*3+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="5", command=lambda x: self.entry_var.set(self.entry_var.get() + "5")).place(x=BUTTON_WIDTH + 1, y=LABEL_HEIGHT*3+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="6", command=lambda x: self.entry_var.set(self.entry_var.get() + "6")).place(x=BUTTON_WIDTH*2+2, y=LABEL_HEIGHT*3+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="*", command=lambda x: self.entry_var.set(self.entry_var.get() + "*")).place(x=BUTTON_WIDTH*3+3, y=LABEL_HEIGHT*3+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="x**y", command=lambda x: self.entry_var.set(self.entry_var.get() + "**")).place(x=BUTTON_WIDTH*4+4, y=LABEL_HEIGHT*3+3, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)

        Button(frame, text="1", command=lambda x: self.entry_var.set(self.entry_var.get() + "1")).place(x=0, y=LABEL_HEIGHT*4+4, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="2", command=lambda x: self.entry_var.set(self.entry_var.get() + "2")).place(x=BUTTON_WIDTH + 1, y=LABEL_HEIGHT*4+4, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="3", command=lambda x: self.entry_var.set(self.entry_var.get() + "3")).place(x=BUTTON_WIDTH*2+2, y=LABEL_HEIGHT*4+4, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="-", command=lambda x: self.entry_var.set(self.entry_var.get() + "-")).place(x=BUTTON_WIDTH*3+3, y=LABEL_HEIGHT*4+4, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="=", command=lambda x: calc_(self.master, self.entry_var)).place(x=BUTTON_WIDTH*4+4, y=LABEL_HEIGHT*4+4, width=BUTTON_WIDTH, height=BUTTON_HEIGHT*2+1)

        Button(frame, text="0", command=lambda x: self.entry_var.set(self.entry_var.get() + "0")).place(x=0, y=LABEL_HEIGHT*5+5, width=BUTTON_WIDTH*2, height=BUTTON_HEIGHT)
        Button(frame, text=".", command=lambda x: self.entry_var.set(self.entry_var.get() + ".")).place(x=BUTTON_WIDTH*2+2, y=LABEL_HEIGHT*5+5, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
        Button(frame, text="+", command=lambda x: self.entry_var.set(self.entry_var.get() + "+")).place(x=BUTTON_WIDTH*3+3, y=LABEL_HEIGHT*5+5, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
