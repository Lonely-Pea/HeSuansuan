...
# 函数 - 求三角函数
def triangle_fucntions_calc(master, var_angle, var_answer_sin, var_answer_cos, var_answer_tan):
    """
    使用math.sin(), math.cos(), math.tan()计算
    
    """
    try:
        angle = float(var_angle.get())
        sin, cos, tan = str("%.5f" % math.sin(angle*math.pi/180)), str("%.5f" % math.cos(angle*math.pi/180)), str("%.5f" % math.tan(angle*math.pi/180))
        var_answer_sin.set(sin)
        var_answer_cos.set(cos)
        var_answer_tan.set(tan)

    except Exception:
        bool_shine(master, vars=[var_angle])


# 计算器
# 计算器 - 计算
def calc_(master, var, entry):
    try:
        get_ = var.get()
        var.set("")
        entry.insert("end", eval(get_))
...
# 计算器 - 计算阶乘
def calc_jiecheng(master, var, entry):
    try:
        get_ = var.get()
        var.set("")
        entry.insert("end", math.factorial(
                float(
                    eval(
                        get_
                        )
...
# 计算器 - 计算自然对数
def calc_log(master, var, entry):
    print("calc log is running")
    try:
        get_ = var.get()
        var.set("")
        number = float(
        ...
        entry.insert("end", math.log(number))

    except Exception:
        bool_shine(master, vars=[var])

    # finally:
        # print("calc log finished")


# 计算器 - 计算三角函数
def triangle_fucntions_calc(master, var, way, entry):
    """
    使用math.sin(), math.cos(), math.tan()计算
    ...
    try:
        ...
        if way == 1:
            answer = str("%.5f" % math.sin(math.radians(angle)))

        elif way == 2:
            answer = str("%.5f" % math.cos(math.radians(angle)))

        elif way == 3:
            answer = str("%.5f" % math.tan(math.radians(angle)))

        elif way == 4:
            answer = str("%.5f" % math.degrees(math.asin(angle)))

        elif way == 5:
            answer = str("%.5f" % math.degrees(math.acos(angle)))

        elif way == 6:
            answer = str("%.5f" % math.degrees(math.atan(angle)))
...
# 计算器 - 显示π
def pi_display(master, var, entry):
    var.set("")
    entry.insert("end", "3.1415926535897932384626433832795")
